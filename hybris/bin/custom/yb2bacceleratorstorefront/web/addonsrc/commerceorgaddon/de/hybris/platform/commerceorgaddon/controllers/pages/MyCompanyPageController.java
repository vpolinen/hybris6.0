...
public class MyCompanyPageController extends AbstractSearchPageController
{
	...
	{
		return baseStoreService.getCurrentBaseStore() == null ? StringUtils.EMPTY : baseStoreService.getCurrentBaseStore()
				.getName();
	}

	@RequestMapping(value = "/my-company", method = RequestMethod.GET)
	@RequireHardLogIn
	public String myCompany(final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
	...
	@RequireHardLogIn
	public String organizationManagement(final Model model, final RedirectAttributes redirectModel)
			throws CMSItemNotFoundException
	{
		return myCompany(model, redirectModel);
	}
	...
	protected String editCostCenterDetails(final String costCenterCode, final Model model) throws CMSItemNotFoundException
	{
		...
		final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageCostCenterBreadCrumbs();
		myCompanyBreadcrumbBuilder.addViewCostCenterBreadCrumbs(breadcrumbs, costCenterCode);
		breadcrumbs.add(new Breadcrumb(String.format(MANAGE_COSTCENTERS_EDIT_URL, urlEncode(costCenterCode)), getMessageSource()
				.getMessage("text.company.costCenter.edit.title", null, getI18nService().getCurrentLocale()), null));
		model.addAttribute("breadcrumbs", breadcrumbs);
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return ControllerConstants.Views.Pages.MyCompany.MyCompanyCostCenterAddPage;
	}
