...
<template:page pageTitle="${pageTitle}">
                    ...
                    <div class="account-list">
                        <div class="account-cards card-select">
                            <c:forEach items="${searchPageData.results}" var="result">
                                ...
                                <div id="row-${ycommerce:normalizedCode(result.uid)}" class="card col-xs-12 col-sm-6 col-md-4 <c:if test='${result.selected}'>selected</c:if>">
                                    <ul>
                                        <li>
    ...
    <script id="enableDisableLinksTemplate" type="text/x-jquery-tmpl">
        {{if selected}}
            <a href="#" url="${actionLink}/deselect/?usergroup={{= id}}&user=${param.user}" class="action-links js-deselectUser"><span class="glyphicon glyphicon-ok"></span></a>
        {{else}}
            <a href="#" url="${actionLink}/select/?usergroup={{= id}}&user=${param.user}" class="action-links js-selectUser"><span class="glyphicon glyphicon-ok"></span></a>
        {{/if}}
    </script>
</template:page>