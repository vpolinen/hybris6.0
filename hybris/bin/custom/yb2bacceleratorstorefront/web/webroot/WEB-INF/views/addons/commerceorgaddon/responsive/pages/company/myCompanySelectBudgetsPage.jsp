...
<template:page pageTitle="${pageTitle}">
                    ...
                    <div class="account-list">
                        <div class="account-cards card-select">
                            <c:forEach items="${searchPageData.results}" var="b2bBudget">
                                <div id="card-${ycommerce:normalizedCode(b2bBudget.code)}" class="card col-xs-12 col-sm-6 col-md-4 <c:if test='${b2bBudget.selected}'>selected</c:if>">
                                    <ul class="card-${ycommerce:normalizedCode(b2bBudget.code)} pull-left col-xs-10">
                                        <spring:url
                                        ...
                                        <li><a href="${viewBudgetUrl}">${fn:escapeXml(b2bBudget.code)}</a></li>
                                    ...
                                    <span id="span-${ycommerce:normalizedCode(b2bBudget.code)}" class="account-cards-actions">
                                        ...
                                        <c:choose>
                                            <c:when test="${b2bBudget.selected}">
                                                <a href="#" url="${deselectUrl}" class="action-links js-deselectBudget">
                                                    <span class="glyphicon glyphicon-ok"></span>
                                                </a>
                                            </c:when>
                                            <c:when test="${b2bBudget.active }">
                                                <a href="#" url="${selectUrl}" class="action-links js-selectBudget">
                                                    <span class="glyphicon glyphicon-ok"></span>
                                                </a>
                                            </c:when>
                                        </c:choose>
    ...
    {{if active}}
		{{if selected}}
			<a href="#" url="${budgetsActionLink}/deselect/?budgetCode={{= id}}&costCenterCode=${ycommerce:encodeUrl(param.costCenterCode)}" class="action-links js-deselectBudget"><span class="glyphicon glyphicon-ok"></span></a>
		{{else}}
			<a href="#" url="${budgetsActionLink}/select/?budgetCode={{= id}}&costCenterCode=${ycommerce:encodeUrl(param.costCenterCode)}" class="action-links js-selectBudget"><span class="glyphicon glyphicon-ok"></span></a>
		{{/if}}
		{{else}}
	{{/if}}
    </script>
