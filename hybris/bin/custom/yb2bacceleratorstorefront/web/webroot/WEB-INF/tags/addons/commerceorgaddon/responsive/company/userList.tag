...
<div class="account-list-header">
    ...
    <div class="account-list-header-add pull-right">
        <div class="hidden-xs">
            <a href="${editUrl}" class="button edit">
                <spring:theme code="text.company.addExisting.button"/>
            </a>
            |
            <a href="${createUrl}&role=${role}" class="button create">
    ...
    <c:forEach items="${users}" var="user" varStatus="rows">
        <div class="card col-xs-12 col-sm-6 col-md-4">
            ...
            <div class="account-cards-actions">
                <a href="#" url="${removeUserUrl}" class="js-remove-user-from-unit remove-item" data-rowid="${rows.index}" title="<spring:theme code='text.company.manage.unit.user.remove'/>">
