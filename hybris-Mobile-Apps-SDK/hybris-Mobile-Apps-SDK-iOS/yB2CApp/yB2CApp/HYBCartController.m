...
- (void)viewDidLoad {
    ...
    [self registerForCartChangeNotifications:@selector(loadCurrentCart)
                                senderObject:self.backendService];
...
- (void)deleteItemAtIndex:(NSIndexPath*)indexPath {
    [self.backendService updateProductOnCurrentCartAmount:[NSString stringWithFormat:@"%ld", (long)indexPath.row]
                                                mount:@0
...
- (void)productCellTapped:(UITapGestureRecognizer *)tapRecognizer {
    ...
    if (!self.mainView.itemsTable.editing) {
        ...
        [self.backendService getProductForCode:cell.productCode andExecute:^(HYBProduct *product, NSError *error) {
...
- (void)itemsQuantityChanged:(UITextField*)sender {
    ...
    if (amount.intValue == 0) {
        ...
        if (amount) {
            [self.backendService updateProductOnCurrentCartAmount:[NSString stringWithFormat:@"%ld", (long) self.currentlyEditedCartItemPosition] mount:amount andExecute:^(HYBCart *cart, NSError *error) {
...
- (void)deleteCurrentlyEditedItem {
    [self.backendService updateProductOnCurrentCartAmount:[NSString stringWithFormat:@"%ld", (long) self.currentlyEditedCartItemPosition] mount:@0 andExecute:^(HYBCart *cart,  NSError *error) {
        DDLogDebug(@"Product on cart was updated.");
        self.currentlyEditedCartItemPosition = -1;
